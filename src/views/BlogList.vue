<template>
  <div>
    <h1 style="margin-bottom:2rem; font-size:1.5rem;">博客列表</h1>
    <el-input
      v-model="keyword"
      size="mini"
      @input="handleInput"
...
export default {
  ...
  methods: {
    async fetch() {
      const res = await this.$axios.get("/blog/getBlogs");
      console.log(res);
    ...
    async handleInput(value) {
      if (value !== "") {
        const res = await this.$axios.get(`/blog/search/${this.keyword}`);
        this.tableData = res.data;
    ...
    handleEdit(index, { _id }) {
      ...
      this.$router.push(`/createBlog/${_id}`);
    },
    async handleDelete(index, { title, _id }) {
      console.log(index, _id);
      await this.$axios.delete(`/blog/delBlog/${_id}`);
      try {
    ...
    async handlePageOpt(pageSize, page) {
      const res = await this.$axios.get(`/blog/getBlogs/${pageSize}/${page}`);
      console.log(res);
